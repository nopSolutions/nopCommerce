    ...
    public class HostedPaymentProcessor : IPaymentMethod
    {
        ...
        public string PostProcessPayment(Order order)
        {
            ...
            for(int i = 0; i < opvCollection.Count; i++, rowNumber++)
            {
                ...
                sb.AppendFormat("Row{0}VATPercentage={1}&", rowNumber, (int)IoC.Resolve<ITaxService>().GetTaxRate(pv, customer, ref errStr));
            }
            ...
            sb.AppendFormat("{0}={1}", "Currency", IoC.Resolve<ICurrencyService>().PrimaryStoreCurrency.CurrencyCode);
