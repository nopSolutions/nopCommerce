...
namespace NopSolutions.NopCommerce.Payment.Methods.Worldpay
{
    ...
    public class WorldpayPaymentProcessor : IPaymentMethod
    {
        ...
        {
            useSandBox = IoC.Resolve<ISettingManager>().GetSettingValueBoolean(WorldpayConstants.SETTING_USE_SANDBOX);
            instanceID = IoC.Resolve<ISettingManager>().GetSettingValue(WorldpayConstants.SETTING_INSTANCEID);
        }
        #endregion
        ...
        {
            return IoC.Resolve<ISettingManager>().GetSettingValueDecimalNative("PaymentMethod.Worldpay.AdditionalFee");
        }
