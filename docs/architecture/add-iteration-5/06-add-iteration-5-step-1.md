...
| Concern | Description |
| --- | --- |
| CON-25 | Correlation: the polling task must match an OpenBoxes fulfillment order back to a nopCommerce order. The `OrderGuid` was sent to OpenBoxes by the bridge (ADR-008/ADR-009); the polling task needs to read it back from OpenBoxes' response |
