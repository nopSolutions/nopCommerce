...
namespace Nop.Plugin.Api.Services
{
    ...
    {
        private static readonly string LanguageIdAttribute = "Language.Id".ToLowerInvariant();
        private static readonly string CurrencyIdAttribute = "Currency.Id".ToLowerInvariant();
		...
		public async Task<Language> GetCustomerLanguageAsync(Customer customer)
		{
            ...
            var customerLanguageId = await _genericAttributeService.GetAttributeAsync<int>(customer, LanguageIdAttribute/*, store.Id*/);
		...
		{
            //var store = await _storeContext.GetCurrentStoreAsync();
            await _genericAttributeService.SaveAttributeAsync(customer, LanguageIdAttribute, language?.Id ?? 0/*, store.Id*/);
        }
		...
		public async Task<Currency> GetCustomerCurrencyAsync(Customer customer)
		{
            ...
            var customerCurrencyId = await _genericAttributeService.GetAttributeAsync<int>(customer, CurrencyIdAttribute/*, store.Id*/);
		...
		{
            //var store = await _storeContext.GetCurrentStoreAsync();
            await _genericAttributeService.SaveAttributeAsync(customer, CurrencyIdAttribute, currency?.Id ?? 0/*, store.Id*/);
        }
	}
}