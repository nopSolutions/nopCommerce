    ...
    public class AuthModelValidator : BaseNopValidator<AuthModel>
    {
        public AuthModelValidator(ILocalizationService localizationService)
        {
            RuleFor(model => model.Code).NotEmpty()
                .WithMessage(localizationService.GetResourceAsync("Plugins.MultiFactorAuth.GoogleAuthenticator.Fields.Code.Required"));
            RuleFor(model => model.Code).Matches(@"^[0-9]{6,6}$")
                .WithMessage(localizationService.GetResourceAsync("Plugins.MultiFactorAuth.GoogleAuthenticator.Fields.Code.Wrong"));
        }
    }
}
