...
public class ConfigurationValidator : BaseNopValidator<FacebookPixelModel>
{
    ...
    public ConfigurationValidator(ILocalizationService localizationService)
    {
        ...
        RuleFor(model => model.PixelId)
            .NotEmpty()
            .WithMessage("Plugins.Widgets.FacebookPixel.Configuration.Fields.PixelId.Required");
        RuleFor(model => model.AccessToken)
            .NotEmpty()
            .WithMessage("Plugins.Widgets.FacebookPixel.Configuration.Fields.AccessToken.Required")
            .When(model => model.ConversionsApiEnabled);
        RuleFor(model => model.UseAdvancedMatching)
            .NotEqual(true)
            .WithMessage("Plugins.Widgets.FacebookPixel.Configuration.Fields.UseAdvancedMatching.Forbidden")
            .When(model => model.PassUserProperties);
        RuleFor(model => model.PassUserProperties)
            .NotEqual(true)
            .WithMessage("Plugins.Widgets.FacebookPixel.Configuration.Fields.PassUserProperties.Forbidden")
