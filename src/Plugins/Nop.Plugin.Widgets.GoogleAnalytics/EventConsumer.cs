        ...
        try
        {
            ...
            if (product is { CallForPrice: false })
            {
                ...
                var (productPrice, _) = await _taxService
                    .GetProductPriceAsync(product, sciUnitPrice, googleAnalyticsSettings.IncludingTax, customer);
            ...
            var items = new List<Item>();
            var sku = product is not null
                ? await _productService.FormatSkuAsync(product, shoppingCartItem.AttributesXml)
                : null;
