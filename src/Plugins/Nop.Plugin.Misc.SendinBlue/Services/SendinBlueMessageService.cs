...
namespace Nop.Plugin.Misc.SendinBlue.Services
{
    ...
    public class SendinBlueMessageService : WorkflowMessageService
    {
        ...
        private async Task SendSmsNotification(MessageTemplate messageTemplate, IEnumerable<Token> tokens)
        {
            //get plugin settings
            var storeId = (int?)tokens.FirstOrDefault(token => token.Key == "Store.Id")?.Value;
            var sendinBlueSettings = await _settingService.LoadSetting<SendinBlueSettings>(storeId ?? 0);
        ...
        private async Task<int?> SendEmailNotification(MessageTemplate messageTemplate, EmailAccount emailAccount, IEnumerable<Token> tokens,
            ...
            string fromEmail = null, string fromName = null, string subject = null)
        {
            ...
            emailAccount = await _emailAccountService.GetEmailAccountById(sendinBlueSettings.EmailAccountId) ?? emailAccount;

            //get an email from the template
            var email = await _sendinBlueEmailManager.GetQueuedEmailFromTemplate(templateId.Value)
        ...
        /// <param name="subject">Subject. If specified, then it overrides subject of a message template</param>
        /// <returns>Queued email identifier</returns>
        public override async Task<int> SendNotification(MessageTemplate messageTemplate, EmailAccount emailAccount, int languageId, IList<Token> tokens,
            ...
            string subject = null)
        {
            ...
            await SendSmsNotification(messageTemplate, tokens);

            //try to send email notification
            var emailId = await SendEmailNotification(messageTemplate, emailAccount, tokens,
            ...
            return await base.SendNotification(messageTemplate, emailAccount, languageId, tokens,
                toEmailAddress, toName, attachmentFilePath, attachmentFileName,
                replyToEmailAddress, replyToName, fromEmail, fromName, subject);
        }
