...
public class GoogleAiService
{
    ...
    private async Task<IEnumerable<string>> GetProductCategoriesAsync(Product product, IList<Category> allCategories)
    {
        ...
        return await categories
            .SelectAwait(async category => CommonHelper.EnsureMaximumLength(await _categoryService
                .GetFormattedBreadCrumbAsync(category, allCategories, GoogleAiDefaults.CategorySeparator, _localizationSettings.DefaultAdminLanguageId), 5000))
            .ToListAsync();
    }

    /// <summary>
    /// Gets the availability status of a product based on its inventory management method and stock quantity
    /// </summary>
    /// <param name="product">The product for which to get availability status</param>
    /// <returns>
    /// A task that represents the asynchronous operation
    /// The task result contains the availability status
    /// </returns>
    private async Task<Google.Cloud.Retail.V2.Product.Types.Availability> GetAvailabilityAsync(Product product)
    {
        ...
        switch (product.ManageInventoryMethod)
        {
            ...
            case ManageInventoryMethod.DontManageStock:
                ...
                break;
            default:
                throw new ArgumentOutOfRangeException(nameof(product.ManageInventoryMethod), product.ManageInventoryMethod, null);
        }
    ...
    private async Task<IEnumerable<string>> GetProductTagsAsync(Product product)
    {
        ...
        return tags.Select(tag => CommonHelper.EnsureMaximumLength(tag.Name, 1000));
    }
    ...
    /// <returns>
    /// A task that represents the asynchronous operation
    /// The task result contains the collection of strings representing the manufacturer names
    /// </returns>
    private async Task<IEnumerable<string>> GetProductManufacturersAsync(Product product, IList<Manufacturer> allManufacturers)
    {
        ...
        return manufacturers.Select(m => CommonHelper.EnsureMaximumLength(m.Name, 1000));
    }
    ...
    private async Task<Google.Cloud.Retail.V2.Product> ConvertProductToGoogleProductAsync(Product product, IList<Category> allCategories, IList<Manufacturer> allManufacturers, Language currentLanguage = null)
    {
        ...
        var dto = new Google.Cloud.Retail.V2.Product
        {
            ...
            Title = CommonHelper.EnsureMaximumLength(product.ShortDescription ?? string.Empty, 1000),
            Description = CommonHelper.EnsureMaximumLength(product.FullDescription ?? string.Empty, 5000),
            ...
            Uri = await _nopUrlHelper.RouteGenericUrlAsync(product, _webHelper.GetCurrentRequestProtocol(), languageId: currentLanguage.Id),
            Rating = new Rating
            {
            ...
            Availability = await GetAvailabilityAsync(product)
        };
        ...
        ProductAttributeCombination combination, Product product)
    {
        ...
        var dto = new Google.Cloud.Retail.V2.Product
        {
            Gtin = combination.Gtin ?? string.Empty,
            Name = product.Name,
            Id = string.IsNullOrEmpty(combination.Sku) ? combination.Id.ToString() : combination.Sku,
    ...
    private async Task<ImportProductsRequest> GetImportProductsRequestAsync(IEnumerable<Google.Cloud.Retail.V2.Product> productsToImport)
    {
        ...
        await _logger.InformationAsync($"{GoogleAiDefaults.SystemName}. Sync products with Google Cloud Retail");
    ...
    /// <summary>
    /// Get delete product combination request.
    /// </summary>
    /// <param name="combination">The product combination to delete</param>
    /// <returns>
    /// A task that represents the asynchronous operation
    /// The task result contains the delete product combination request
    /// </returns>
    private async Task<DeleteProductRequest> GetDeleteProductCombinationRequestAsync(ProductAttributeCombination combination)
    {
        var productName = new ProductName(_googleAiSettings.ProjectId, _googleAiSettings.LocationId, _googleAiSettings.CatalogId, _googleAiSettings.BranchId, string.IsNullOrEmpty(combination.Sku) ? combination.Id.ToString() : combination.Sku).ToString();
        var deleteProductRequest = new DeleteProductRequest
        {
            Name = productName
        };
    ...
    private async Task<IDictionary<string, CustomAttribute>> GetSpecificationAttributesAsync(int productId)
    {
        ...
        foreach (var spec in options.GroupBy(o => o.SpecificationAttributeId))
        {
            ...
            result.Add(CommonHelper.EnsureMaximumLength(attribute.Name, 128), value);
        }
    ...
    /// <returns>
    /// A task that represents the asynchronous operation
    /// The task result contains a filter string
    /// </returns>
    private async Task<StringBuilder> CreateFilterAsync(IList<int> categoryIds, IList<int> manufacturerIds, int productTagId, IList<SpecificationAttributeOption> filteredSpecOptions)
    {
        ...
        if (categoryIds?.Any() ?? false)
        {
            ...
            var allCategories = await _categoryService.GetAllCategoriesAsync();
            filter.Append($"category:({string.Join(" OR ", categories.SelectAwait(async c => await _categoryService.GetFormattedBreadCrumbAsync(c, allCategories, GoogleAiDefaults.CategorySeparator, _localizationSettings.DefaultAdminLanguageId)))})");
        }

        if (manufacturerIds?.Any(id => id != 0) ?? false)
        {
            if (filter.Length > 0)
                filter.Append(" AND ");
    ...
    private async Task LogRequestAsync(object request)
    {
        ...
        sb.AppendLine($"{GoogleAiDefaults.SystemName}. Request:");
        ...
        int pageSize)
    {
        ...
        var searchRequest = new SearchRequest
        {
            ...
            Query = keywords,
            VisitorId = clientId ?? string.Empty,
            PageSize = int.Min(pageSize, GoogleAiDefaults.SearchRequestCount),
    ...
    /// <returns>
    /// A task that represents the asynchronous operation
    /// The task result contains the number of success and failure requests
    /// </returns>
    public async Task<(long successCount, long failureCount)> SyncProductsAsync()
    {
        ...
        while (true)
        {
            var nopProducts = await _productService.SearchProductsAsync(page++, _googleAiSettings.ImportPageSize);
        ...
        var sb = new StringBuilder();
        sb.AppendLine($"{GoogleAiDefaults.SystemName}. Import products operation is done");
    ...
    {
        var deleteProductRequest = await GetDeleteProductCombinationRequestAsync(productAttributeCombination);
        var client = await ProductServiceClient.CreateAsync();
    ...
    /// </returns>
    public async Task<(List<int>, int)> SearchProductsAsync(string keywords,
    ...
    {
        if (!_googleAiSettings.SearchAllowed || !_googleAiSettings.Enabled)
            return ([], 0);
        ...
        var searchRequest =
            await GetSearchRequestAsync(keywords, categoryIds, manufacturerIds, productTagId, filteredSpecOptions, pageIndex, pageSize);
        ...
        try
        {
            ...
            if (!skus.Any())
                return (result, firstPage.TotalSize);

            var products = await _productService.GetProductsBySkuAsync(skus.ToArray());
            result.AddRange(products.Select(p => p.Id));
            var combinations = await _productAttributeService.GetProductAttributeCombinationsBySkuAsync(skus.ToArray());
            result.AddRange(combinations.Select(combination => combination.ProductId));
        ...
        {
            await _logger.ErrorAsync($"{GoogleAiDefaults.SystemName}. Error:", ex);
            throw;
        }
    }
