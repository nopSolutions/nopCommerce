...
/// <summary>
/// Represents Google AI Commerce Search engine plugin
/// </summary>
public class GoogleAiPlugin : BasePlugin, IMiscPlugin, IAiRecommendationPlugin
{
    #region Fields

    private readonly ArtificialIntelligenceSettings _artificialIntelligenceSettings;
    ...
    /// </returns>
    public async Task<(List<int>, int)> SearchProductsAsync(string keywords, IList<int> categoryIds = null,
        IList<int> manufacturerIds = null, int productTagId = 0,
        IList<SpecificationAttributeOption> filteredSpecOptions = null, int pageIndex = 0,
        int pageSize = int.MaxValue)
    {
        return await _googleAiService.SearchProductsAsync(keywords, categoryIds, manufacturerIds, productTagId, filteredSpecOptions, pageIndex, pageSize);
    }
    ...
    public override async Task InstallAsync()
    {
        ...
        await _settingService.SaveSettingAsync(new GoogleAiSettings
        {
            Enabled = false,
            LocationId = "global",
            ...
            SearchAllowed = true,
            SyncAllowed = true,
            ImportPageSize = GoogleAiDefaults.ImportPageSize
        });
