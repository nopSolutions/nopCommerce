    ...
    IConsumer<RouteUrlEvent>
{
    ...
    /// <summary>
    /// Handle route URL event
    /// </summary>
    /// <param name="eventMessage">Event message</param>
    /// <returns>A task that represents the asynchronous operation</returns>
    public Task HandleEventAsync(RouteUrlEvent eventMessage)
    {
        if (eventMessage.EntityType == typeof(NewsItem))
        {
            eventMessage.Url = _nopUrlHelper.RouteUrl(NewsDefaults.Routes.Public.NewsItemRouteName,
                eventMessage.Values,
                eventMessage.Protocol,
                eventMessage.Host,
                eventMessage.Fragment);
            eventMessage.StopProcessing = true;
        }
