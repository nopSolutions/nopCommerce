...
public class NewsItemValidator : BaseNopValidator<NewsItemModel>
{
    public NewsItemValidator(ILocalizationService localizationService)
    {
        RuleFor(x => x.Title)
            .NotEmpty()
            .WithMessage("Plugins.Misc.News.NewsItems.Fields.Title.Required");

        RuleFor(x => x.Short)
            .NotEmpty()
            .WithMessage("Plugins.Misc.News.NewsItems.Fields.Short.Required");

        RuleFor(x => x.Full)
            .NotEmpty()
            .WithMessage("Plugins.Misc.News.NewsItems.Fields.Full.Required");
