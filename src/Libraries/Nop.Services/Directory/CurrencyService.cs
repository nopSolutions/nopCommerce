...
public partial class CurrencyService : ICurrencyService
{
    ...
    {
        Arguments.NotNull(sourceCurrencyCode, nameof(sourceCurrencyCode));
        Arguments.NotNull(targetCurrencyCode, nameof(targetCurrencyCode));

        return await new MoneyConverter(amount, sourceCurrencyCode, targetCurrencyCode)
            .ConvertAsync(async conversion =>
            {
                decimal asPrimary = await ConvertToPrimaryExchangeRateCurrencyAsync(conversion.Amount, conversion.Source);
                return await ConvertFromPrimaryExchangeRateCurrencyAsync(asPrimary, conversion.Target);
            });
    }
    ...
    public virtual async Task<decimal> ConvertToPrimaryExchangeRateCurrencyAsync(decimal amount, Currency sourceCurrencyCode)
    {
        ...
        return (await GetPrimaryCurrencyAsync())
            .Map(primaryCurrencyCode => new MoneyConverter(amount, sourceCurrencyCode, primaryCurrencyCode))
            .Map(request => request.ConvertForward().ValueOr(() => ThrowExchangeRateNotFound(request.Source)))
            .ValueOr(() => ThrowPrimaryExchangeRateNotFound());
    }
    ...
    public virtual async Task<decimal> ConvertFromPrimaryExchangeRateCurrencyAsync(decimal amount, Currency targetCurrencyCode)
    {
        ...
        return (await GetPrimaryCurrencyAsync())
            .Map(primaryCurrencyCode => new MoneyConverter(amount, primaryCurrencyCode, targetCurrencyCode))
            .Map(request => request.ConvertBackward().ValueOr(() => ThrowExchangeRateNotFound(request.Target)))
            .ValueOr(() => ThrowPrimaryExchangeRateNotFound());
    }

    private async Task<Option<Currency>> GetPrimaryCurrencyAsync()
        => await GetCurrencyByIdAsyncOrNoneAsync(_currencySettings.PrimaryExchangeRateCurrencyId);

    private async Task<Option<Currency>> GetCurrencyByIdAsyncOrNoneAsync(int currencyId)
        => (await GetCurrencyByIdAsync(currencyId)).SomeNotNull();

    private static decimal ThrowExchangeRateNotFound(Currency currency)
        => throw new NopException($"Exchange rate not found for currency [{currency.Name}]");

    private static decimal ThrowPrimaryExchangeRateNotFound()
