...
public partial class InstallationService
{
    ...
    {
        var eaGeneral = Table<EmailAccount>().FirstOrDefault() ?? throw new Exception("Default email account cannot be loaded");
    ...
    protected virtual async Task InstallSettingsAsync(RegionInfo regionInfo)
    {
        ...
        await settingService.SaveSettingAsync(new CurrencySettings
        {
            ...
            PrimaryStoreCurrencyId = Table<Currency>().Single(c => c.CurrencyCode == primaryCurrency).Id,
            PrimaryExchangeRateCurrencyId = Table<Currency>().Single(c => c.CurrencyCode == primaryCurrency).Id,
        ...
        {
            BaseDimensionId = Table<MeasureDimension>().Single(m => m.SystemKeyword == baseDimension).Id,
            BaseWeightId = Table<MeasureWeight>().Single(m => m.SystemKeyword == baseWeight).Id
        });
    ...
    {
        var defaultTopicTemplate = Table<TopicTemplate>().FirstOrDefault(tt => tt.Name == "Default template") ?? throw new Exception("Topic template cannot be loaded");
