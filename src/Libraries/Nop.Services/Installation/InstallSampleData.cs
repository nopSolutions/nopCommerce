...
using Nop.Services.Installation.JsonModels;
...
public partial class InstallationService
{
    ...
    /// A task that represents the asynchronous operation
    /// </returns>
    protected virtual async Task InstallSampleCustomersAsync(string sampleData)
    {
        var jsonModels = JsonConvert.DeserializeObject<List<CustomerJsonModel>>(sampleData);
        ...
        (Customer Customer, CustomerJsonModel jsonModel) createCustomer((Address address, CustomerJsonModel jsonModel) data)
        {
        ...
        async Task<(Address Address, CustomerJsonModel jsonModel)> createAddress(CustomerJsonModel model)
        {
            return (new Address
            {
                ...
                City = model.City,
                StateProvinceId = await GetFirstEntityIdAsync<StateProvince>(sp => sp.Name == model.StateProvince),
                CountryId = await GetFirstEntityIdAsync<Country>(c => c.ThreeLetterIsoCode == model.CountryThreeLetterIsoCode),
                ZipPostalCode = model.ZipPostalCode,
                CreatedOnUtc = DateTime.UtcNow
            }, model);
        }

        var addresses = await jsonModels.SelectAwait(async c => await createAddress(c)).ToListAsync();
