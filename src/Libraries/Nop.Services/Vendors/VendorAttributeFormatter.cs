...
namespace Nop.Services.Vendors
{
    ...
    public partial class VendorAttributeFormatter : IVendorAttributeFormatter
    {
        ...
        public virtual async Task<string> FormatAttributes(string attributesXml, string separator = "<br />", bool htmlEncode = true)
        {
            var result = new StringBuilder();

            var attributes = await _vendorAttributeParser.ParseVendorAttributes(attributesXml);
            for (var i = 0; i < attributes.Count; i++)
            {
                ...
                for (var j = 0; j < valuesStr.Count; j++)
                {
                    ...
                    else
                    {
                        ...
                        {
                            var attributeValue = await _vendorAttributeService.GetVendorAttributeValueById(attributeValueId);
