...
namespace Nop.Services.ExportImport
{
    ...
    public partial class ExportManager : IExportManager
    {
        ...
        {
            var productProperties = GetProductProperies();
            var productManager = new PropertyManager<Product>(productProperties.Where(p=>!p.Ignore));
            ...
            using (var stream = new MemoryStream())
            {
                ...
                using (var xlPackage = new ExcelPackage(stream))
                {
                    ...
                    foreach (var order in itemsToExport)
                    {
                        ...
                        //products
                        var products = order.OrderItems.Select(o => o.Product).ToList();
                        ...
                        productManager.WriteCaption(worksheet, SetCaptionStyle, row, 2);
                        worksheet.Row(row).OutlineLevel = 1;
                        worksheet.Row(row).Collapsed = true;
                        ...
                        {
                            row++;
                            productManager.CurrentObject = product;
                            productManager.WriteToXlsx(worksheet, row, _catalogSettings.ExportImportUseDropdownlistsForAssociatedEntities, 2, fpWorksheet);
                            worksheet.Row(row).OutlineLevel = 1;
                            worksheet.Row(row).Collapsed = true;
                        }
        ...
        protected virtual PropertyByName<Product>[] GetProductProperies()
        {
            var properties = new[]
            {
        ...
        protected virtual void WriteBaseProductInfoToXml(XmlTextWriter xmlWriter, Product product)
        {
        ...
        public virtual string ExportProductsToXml(IList<Product> products)
        {
            ...
            foreach (var product in products)
            {
                ...
                WriteBaseProductInfoToXml(xmlWriter, product);
        ...
        {
            var properties = GetProductProperies();
                ...
                if (_orderSettings.ExportWithProducts)
                {
                    ...
                    if (orderItems.Any())
                    {
                        ...
                        foreach (var orderItem in orderItems)
                        {
                            ...
                            WriteBaseProductInfoToXml(xmlWriter, orderItem.Product);
                            xmlWriter.WriteEndElement();
                        }
                        xmlWriter.WriteEndElement();
                    }
