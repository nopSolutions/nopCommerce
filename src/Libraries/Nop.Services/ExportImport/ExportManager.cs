...
namespace Nop.Services.ExportImport
{
    ...
    public partial class ExportManager : IExportManager
    {
        ...
        {
            return ExporToXlsx(new ManufacturerGetProperties(_pictureService), manufacturers);
        }
        ...
        /// <returns></returns>
        public virtual byte[] ExporToXlsx<T>(IGetProperties<T> getProperty, IEnumerable<T> itemsToExport)
        {
            using (var stream = new MemoryStream())
            {
                ...
                using (var xlPackage = new ExcelPackage(stream))
                {
                    ...
                    var manager = new PropertyManager<T>(getProperty.GetPropertys);
                    manager.WriteCaption(worksheet, SetCaptionStyle);

                    var row = 2;
                    foreach (var category in itemsToExport)
                    {
                        manager.CurentObject = category;
        ...
        {
            return ExporToXlsx(new CategoriesGetProperties(_pictureService), categories);
        }
        ...
        {
            return ExporToXlsx(new ProductGetProperties(_pictureService, _categoryService, _manufacturerService), products);
        }
