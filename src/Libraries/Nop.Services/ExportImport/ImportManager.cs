...
public partial class ImportManager : IImportManager
{
    ...
    protected virtual async Task ImportProductImagesUsingServicesAsync(IList<ProductPictureMetadata> productPictureMetadata)
    {
        ...
        {
            foreach (var picturePath in product.PicturePathes)
            {
    ...
    public virtual async Task ImportProductsFromXlsxAsync(Stream stream)
    {
        ...
        for (var iRow = 2; iRow < metadata.EndRow; iRow++)
        {
            ...
            if (currentVendor != null)
                pictureProperties = pictureProperties.Take(_vendorSettings.MaximumNumberProductPictures).ToList();

            foreach (var propertyName in pictureProperties)
                pictureMetaData.PicturePathes.Add(await DownloadFileAsync(metadata.Manager.GetDefaultProperty(propertyName)?.StringValue, downloadedFiles));
    ...
    protected partial class ProductPictureMetadata
    {
        ...
        public HashSet<string> PicturePathes { get; set; } = new();
