...
public class AddIndexesMigration : ForwardOnlyMigration
{
    ...
    public override void Up()
    {
        ...
        {
            var orders = _dataProvider.GetTable<OrderForMigration>().GroupBy(p => p.OrderGuid, p => p)
                .Where(p => p.Count() > 1)
                .SelectMany(p => p)
                .ToList();
    ...
    #region Nestead classes

    /// <summary>
    /// Represents an order for migration purposes
    ///<remarks>We use it to avoid issues with 5.00 breaking changes</remarks>
    /// </summary>
    public partial class OrderForMigration : BaseEntity, ISoftDeletedEntity
    {
        /// <summary>
    ...
    public class OrderForMigrationNameCompatibility : INameCompatibility
    {
        public Dictionary<Type, string> TableNames =>
            new()
            {
                [typeof(OrderForMigration)] = nameof(Order),
            };
