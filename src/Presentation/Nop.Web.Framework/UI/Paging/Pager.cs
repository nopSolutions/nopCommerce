    ...
    public partial class Pager : IHtmlContent
    {
        ...
        protected virtual async Task<string> GenerateHtmlString()
        {
            ...
            if (_showTotalSummary && (_model.TotalPages > 0))
            {
                ...
                links.Append(string.Format(await localizationService.GetResource("Pager.CurrentPage"),
            ...
            if (_showPagerItems && (_model.TotalPages > 1))
            {
                if (_showFirst)
                {
                    ...
                    if ((_model.PageIndex >= 3) && (_model.TotalPages > _individualPagesDisplayedCount))
                        links.Append(await CreatePageLink(1, await localizationService.GetResource("Pager.First"), _firstPageCssClass));
                }
                ...
                if (_showPrevious)
                {
                    ...
                    if (_model.PageIndex > 0)
                        links.Append(await CreatePageLink(_model.PageIndex, await localizationService.GetResource("Pager.Previous"), _previousPageCssClass));
                }
                ...
                if (_showNext)
                {
                    ...
                    if ((_model.PageIndex + 1) < _model.TotalPages)
                        links.Append(await CreatePageLink(_model.PageIndex + 2, await localizationService.GetResource("Pager.Next"), _nextPageCssClass));
                }
                ...
                if (_showLast)
                {
                    ...
                    if (((_model.PageIndex + 3) < _model.TotalPages) && (_model.TotalPages > _individualPagesDisplayedCount))
                        links.Append(await CreatePageLink(_model.TotalPages, await localizationService.GetResource("Pager.Last"), _lastPageCssClass));
                }
            }
        ...
        protected virtual async Task<string> CreateDefaultUrl(int pageNumber)
        {
            ...
            var url = await webHelper.GetThisPageUrl(false);
            foreach (var routeValue in routeValues)
            {
                url = await webHelper.ModifyQueryString(url, routeValue.Key, routeValue.Value?.ToString());
            }
            ...
            if (_renderEmptyParameters && parametersWithEmptyValues.Any())
            {
                ...
                {
                    url = await webHelper.ModifyQueryString(url, key);
                }
            }
