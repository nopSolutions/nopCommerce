...
public static partial class MigrationExtension
{
    ...
    public static void RenameLocales(this IMigration _, Dictionary<string, string> localesToRename)
    {
        ...
        foreach (var lang in allLanguages)
            ...
            {
                var lsr = getLocaleStringResourceByName(locale.Key, lang.Id, false);

                if (lsr == null)
                    continue;

                var exist = getLocaleStringResourceByName(locale.Value, lang.Id, false);
        ...
        {
            var query = from lsr in dataProvider.GetTable<LocaleStringResource>()
            ...
            if (localeStringResource == null && logIfNotFound)
                syncCodeHelper.InsertEntity(new Log
                {
                    LogLevel = LogLevel.Warning,
                    ShortMessage = $"Resource string ({resourceName}) not found. Language ID = {languageId}",
                    IpAddress = customerSettings.StoreIpAddresses ? webHelper.GetCurrentIpAddress() : string.Empty,
    ...
    public static void AddOrUpdateLocaleResource(this IMigration _, IDictionary<string, string> resources)
    {
        var syncCodeHelper = EngineContext.Current.Resolve<ISyncCodeHelper>();
        var staticCacheManager = EngineContext.Current.Resolve<IStaticCacheManager>();
        ...
        if (resourcesToInsert.Any())
        {
            ...
            var locales = syncCodeHelper.GetAllLanguages(true)
