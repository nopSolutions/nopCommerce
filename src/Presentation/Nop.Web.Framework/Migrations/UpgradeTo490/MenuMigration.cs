...
public class MenuMigration : Migration
{
    ...
    public MenuMigration(IRepository<M.Menu> menuRepository, IRepository<M.MenuItem> menuItemRepository, IRepository<Topic> topicRepository, ISettingService settingService)
    {
    ...
    public override void Up()
    {
        ...
        if (!_menuRepository.Table.Any(m => m.MenuTypeId == (int)MenuType.Footer))
        {
            ...
            _menuItemRepository.Insert(new M.MenuItem
            {
                ...
                EntityId = _topicRepository.Table.FirstOrDefault(t => t.SystemName == "ShippingInfo")?.Id,
            ...
            _menuItemRepository.Insert([
                new()
                {
                    MenuId = footerInformation.Id,
                    MenuItemType = MenuItemType.TopicPage,
                    EntityId = _topicRepository.Table.FirstOrDefault(t => t.SystemName == "PrivacyInfo")?.Id,
                    Published = true
                },
                new()
                {
                    MenuId = footerInformation.Id,
                    MenuItemType = MenuItemType.TopicPage,
                    EntityId = _topicRepository.Table.FirstOrDefault(t => t.SystemName == "ConditionsOfUse")?.Id,
                    Published = true
                },
                new()
                {
                    MenuId = footerInformation.Id,
                    MenuItemType = MenuItemType.TopicPage,
                    EntityId = _topicRepository.Table.FirstOrDefault(t => t.SystemName == "AboutUs")?.Id,
                    Published = true
                }
            ]);
