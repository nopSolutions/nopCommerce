...
namespace Nop.Web.Framework.TagHelpers.Shared
{
    ...
    public class NopDatePickerTagHelper : TagHelper
    {
        ...
        public override void Process(TagHelperContext context, TagHelperOutput output)
        {
            ...
            days.AppendFormat("<option value='{0}'>{1}</option>", "0", locService.GetResourceAsync("Common.Day").Result);
            for (var i = 1; i <= 31; i++)
                days.AppendFormat("<option value='{0}'{1}>{0}</option>", i, 
                    (SelectedDay.HasValue && SelectedDay.Value == i) ? " selected=\"selected\"" : null);

            months.AppendFormat("<option value='{0}'>{1}</option>", "0", locService.GetResourceAsync("Common.Month").Result);
            for (var i = 1; i <= 12; i++)
            {
            ...
            years.AppendFormat("<option value='{0}'>{1}</option>", "0", locService.GetResourceAsync("Common.Year").Result);
