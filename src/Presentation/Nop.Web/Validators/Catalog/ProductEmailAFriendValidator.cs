...
namespace Nop.Web.Validators.Catalog
{
    public partial class ProductEmailAFriendValidator : BaseNopValidator<ProductEmailAFriendModel>
    {
        ...
        {
            RuleFor(x => x.FriendEmail).NotEmpty().WithMessage(localizationService.GetResourceAsync("Products.EmailAFriend.FriendEmail.Required").Result);
            RuleFor(x => x.FriendEmail).EmailAddress().WithMessage(localizationService.GetResourceAsync("Common.WrongEmail").Result);

            RuleFor(x => x.YourEmailAddress).NotEmpty().WithMessage(localizationService.GetResourceAsync("Products.EmailAFriend.YourEmailAddress.Required").Result);
