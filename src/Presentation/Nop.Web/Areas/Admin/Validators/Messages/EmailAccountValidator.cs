...
public partial class EmailAccountValidator : BaseNopValidator<EmailAccountModel>
{
    public EmailAccountValidator(ILocalizationService localizationService)
    {
        ...
        RuleFor(x => x.MaxNumberOfEmails).NotEmpty().GreaterThan(0)
                .WithMessage("Admin.Configuration.EmailAccounts.Fields.MaxNumberOfEmails.ShouldBeGreaterThanZero");

        RuleFor(x => x.ClientId)
            .NotEmpty()
            .WithMessage("Admin.Configuration.EmailAccounts.Fields.ClientId.Required")
        ...
        RuleFor(x => x.TenantId)
            .NotEmpty()
            .WithMessage("Admin.Configuration.EmailAccounts.Fields.TenantId.Required")
