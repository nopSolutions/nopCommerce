...
public partial class CustomerValidator : BaseNopValidator<CustomerModel>
{
        ...
        IStateProvinceService stateProvinceService)
    {
        ...
        RuleFor(x => x.Email)
            ...
            .WhenAsync(async (x, cancellation) => await IsRegisteredCustomerRoleCheckedAsync(x, customerService));
        ...
        if (customerSettings.CountyRequired && customerSettings.CountyEnabled)
        {
            RuleFor(x => x.County)
                .NotEmpty()
                .WithMessage("Admin.Customers.Customers.Fields.County.Required")
                //only for registered users
                .WhenAsync(async (x, cancellation) => await IsRegisteredCustomerRoleCheckedAsync(x, customerService));
        }
