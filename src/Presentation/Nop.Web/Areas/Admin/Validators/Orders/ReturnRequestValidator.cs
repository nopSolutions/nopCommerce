...
{
    public ReturnRequestValidator(ILocalizationService localizationService, ReturnRequestSettings returnRequestSettings)
    {
        if (returnRequestSettings.ReturnReasonsEnabled)
        {
            RuleFor(x => x.ReasonForReturn)
                .NotEmpty()
                .WithMessageAwait(localizationService.GetResourceAsync("Admin.ReturnRequests.Fields.ReasonForReturn.Required"));
        }

        if (returnRequestSettings.ReturnActionsEnabled)
        {
            RuleFor(x => x.RequestedAction)
                .NotEmpty()
                .WithMessageAwait(localizationService.GetResourceAsync("Admin.ReturnRequests.Fields.RequestedAction.Required"));
        }
