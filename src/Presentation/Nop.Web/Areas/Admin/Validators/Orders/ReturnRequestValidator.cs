...
public partial class ReturnRequestValidator : BaseNopValidator<ReturnRequestModel>
{
    ...
    {
        RuleFor(x => x.ReasonForReturn).NotEmpty().WithMessage("Admin.ReturnRequests.Fields.ReasonForReturn.Required");
        RuleFor(x => x.RequestedAction).NotEmpty().WithMessage("Admin.ReturnRequests.Fields.RequestedAction.Required");
        RuleFor(x => x.Quantity).NotEmpty().WithMessage("Admin.ReturnRequests.Fields.Quantity.Required");
        ...
        RuleFor(x => x.ReturnedQuantity).GreaterThan(-1).WithMessage("Admin.ReturnRequests.Fields.ReturnedQuantity.MustBeEqualOrGreaterThanZero");
