...
public partial class MenuItemValidator : BaseNopValidator<MenuItemModel>
{
    ...
    public MenuItemValidator(ILocalizationService localizationService)
    {
        RuleFor(x => x.RouteName)
            ...
            .WithMessage("Admin.ContentManagement.Menus.MenuItem.Fields.RouteName.Required");

        RuleFor(x => x.ProductId)
            .NotNull()
            .When(x => x.MenuItemTypeId == (int)MenuItemType.Product)
            .WithMessage("Admin.ContentManagement.Menus.MenuItem.Fields.Product.Required");

        RuleFor(x => x.TopicId)
            .GreaterThan(0)
            .When(x => x.MenuItemTypeId == (int)MenuItemType.TopicPage)
            .WithMessage("Admin.ContentManagement.Menus.MenuItem.Fields.Topic.Required");
        ...
        RuleFor(x => x.Title)
            ...
            .WithMessage("Admin.ContentManagement.Menus.MenuItem.Fields.Title.Required");

        RuleFor(x => x.Url)
            .NotEmpty()
            .When(x => x.MenuItemTypeId == (int)MenuItemType.CustomLink)
            .WithMessage("Admin.ContentManagement.Menus.MenuItem.Fields.Url.Required");
