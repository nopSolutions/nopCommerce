...
public partial class GdprConsentValidator : BaseNopValidator<GdprConsentModel>
{
    ...
    {
        RuleFor(x => x.Message).NotEmpty().WithMessage("Admin.Configuration.Settings.Gdpr.Consent.Message.Required");
        RuleFor(x => x.RequiredMessage)
            .NotEmpty()
            .WithMessage("Admin.Configuration.Settings.Gdpr.Consent.RequiredMessage.Required")
            .When(x => x.IsRequired);
