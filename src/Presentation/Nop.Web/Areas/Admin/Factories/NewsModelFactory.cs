        ...
        public virtual async Task<NewsItemListModel> PrepareNewsItemListModel(NewsItemSearchModel searchModel)
        {
            ...
            var model = await new NewsItemListModel().PrepareToGridAsync(searchModel, newsItems, () =>
            {
                return newsItems.ToAsyncEnumerable().SelectAwait(async newsItem =>
                {
                    ...
                    newsItemModel.SeName = await _urlRecordService.GetSeName(newsItem, newsItem.LanguageId, true, false);
                    newsItemModel.LanguageName = (await _languageService.GetLanguageById(newsItem.LanguageId))?.Name;
                    newsItemModel.ApprovedComments = await _newsService.GetNewsCommentsCount(newsItem, isApproved: true);
                    newsItemModel.NotApprovedComments = await _newsService.GetNewsCommentsCount(newsItem, isApproved: false);
        ...
        public virtual async Task<NewsCommentListModel> PrepareNewsCommentListModel(NewsCommentSearchModel searchModel, int? newsItemId)
        {
            ...
            var model = await new NewsCommentListModel().PrepareToGridAsync(searchModel, comments, () =>
            {
                return comments.ToAsyncEnumerable().SelectAwait(async newsComment =>
                {
                    ...
                    //fill in additional values (not existing in the entity)
                    commentModel.NewsItemTitle = (await _newsService.GetNewsById(newsComment.NewsItemId))?.Title;

                    if ((await _customerService.GetCustomerById(newsComment.CustomerId)) is Customer customer)
                    {
