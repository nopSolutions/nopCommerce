...
namespace Nop.Web.Areas.Admin.Controllers
{
    ...
    public class RoxyFilemanController : BaseAdminController
    {
        ...
        {
            await _roxyFilemanService.CreateConfiguration();
        }
        ...
        protected virtual async Task ProcessRequestAsync()
        {
            ...
            try
            {
                ...
                switch (action.ToUpper())
                {
                    ...
                    case "RENAMEFILE":
                        ...
                        break;
                    case "GENERATETHUMB":
                        await _roxyFilemanService.CreateImageThumbnail(HttpContext.Request.Query["f"]);
                        break;
                    case "UPLOAD":
            ...
            catch (Exception ex)
            {
                if (action == "UPLOAD" && !_roxyFilemanService.IsAjaxRequest())
                    await HttpContext.Response.WriteAsync($"<script>parent.fileUploaded({_roxyFilemanService.GetErrorResponse(await _roxyFilemanService.GetLanguageResource("E_UploadNoFiles"))});</script>");
                else
