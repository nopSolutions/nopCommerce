...
namespace Nop.Web.Factories
{
    ...
    public partial class BlogModelFactory : IBlogModelFactory
    {
        ...
        public HomepageBlogPostsModel PrepareHomepageBlogPostsModel()
        {

            var cacheKey = _cacheKeyService.PrepareKeyForDefaultCache(NopModelCacheDefaults.HomepageBlogModelKey, _workContext.WorkingLanguage.Id, _storeContext.CurrentStore.Id);
            var cachedModel = _staticCacheManager.Get(cacheKey, () =>
            {
                var blogItems = _blogService.GetAllBlogPosts(_storeContext.CurrentStore.Id, _workContext.WorkingLanguage.Id, null, null, 0, _blogSettings.MainPageBlogPostsCount);

                return new HomepageBlogPostsModel
                {
                    WorkingLanguageId = _workContext.WorkingLanguage.Id,
                    BlogPosts = blogItems
                        .Select(x =>
                        {
                            ...
                            PrepareBlogPostModel(blogModel, x, false);
                            return blogModel;
                        })
                        .ToList()
                };
            ...
            var model = (HomepageBlogPostsModel)cachedModel.Clone();
            foreach (var blogItemModel in model.BlogPosts)
