...
namespace Nop.Web.Factories
{
    ...
    public partial class VendorModelFactory : IVendorModelFactory
    {
        ...
        protected virtual async Task<IList<VendorAttributeModel>> PrepareVendorAttributes(string vendorAttributesXml)
        {
                    ...
                    case AttributeControlType.Checkboxes:
                        {
                            if (!string.IsNullOrEmpty(vendorAttributesXml))
                            {
                                ...
                                //select new values
                                var selectedValues = await _vendorAttributeParser.ParseVendorAttributeValues(vendorAttributesXml);
        ...
        public virtual async Task<ApplyVendorModel> PrepareApplyVendorModel(ApplyVendorModel model,
            bool validateVendor, bool excludeProperties, string vendorAttributesXml)
        {
            if (model == null)
                throw new ArgumentNullException(nameof(model));

            if (validateVendor && (await _workContext.GetCurrentCustomer()).VendorId > 0)
            {
            ...
            //vendor attributes
            model.VendorAttributes = await PrepareVendorAttributes(vendorAttributesXml);
        ...
        public virtual async Task<VendorInfoModel> PrepareVendorInfoModel(VendorInfoModel model,
            bool excludeProperties, string overriddenVendorAttributesXml = "")
        {
            ...
            (model.PictureUrl, _) = picture != null ? await _pictureService.GetPictureUrl(picture, pictureSize) : (string.Empty, null);
            ...
            model.VendorAttributes = await PrepareVendorAttributes(overriddenVendorAttributesXml);
