...
namespace Nop.Admin.Validators.Settings
{
    public class SettingValidator : BaseNopValidator<SettingModel>
    {
        public SettingValidator(ILocalizationService localizationService)
        {
            ...
            RuleFor(x => x.Value).Must(ValidatorUtilities.PageSizeOptionsInAdvancedSettingsValidator).WithMessage(localizationService.GetResource("Admin.Configuration.Settings.AllSettings.Fields.PageSizeOptions.IsInvalid"));
        }
    }
}