    ...
    const tour = window.driver.js.driver({
      ...
      onPopoverRender: (popover, { config, state }) => {
        ...
        if (tour.hasPreviousStep() && localizedData.Back) {
          createPopoverButton(popover, localizedData.Back, "button-back", "fa-chevron-left", () => tour.movePrevious(), false);
        }

        if (tour.hasNextStep() && localizedData.NextStep) {
          createPopoverButton(popover, localizedData.NextStep, "button-next", "fa-chevron-right", () => tour.moveNext(), true);
        }

        if (tour.isLastStep() && nextPageUrl && localizedData.NextPage) {
          createPopoverButton(popover, localizedData.NextPage, "button-next-page", "fa-angle-double-right", () => {
            if (nextPageUrl)
