...
function displayPopupContentFromUrl(url, title, modal, width) {
  ...
  $('<div></div>').load(url)
    .dialog({
      ...
      maxHeight: maxHeight,
      title: title,
      position: { my: "top+300", at: "center top", of: window },
...
function changeWishlistStatus(productId, messagetype) {
  ...
  if (messagetype === 'addToWishList') {
    ...
    //remove product/profile from shortlisted list
    if (productSelector.closest("#tabs-1").length) {
      productSelector.closest("div.item-box").remove()      
    }
