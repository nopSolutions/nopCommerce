...
public partial class ReminderController : BaseAdminController
{
    ...
    public virtual async Task<IActionResult> Index(RemindersModel model)
    {
        if (ModelState.IsValid)
        {
            ...
            await _settingService.SaveSettingAsync(reminderSettings, settings => settings.AbandonedCartEnabled, clearCache: false);
            ...
            if (model.AbandonedCartEnabled && (await _storeService.GetAllStoresAsync()).Count > 1)
                _notificationService.WarningNotification(await _localizationService.GetResourceAsync("Admin.Promotions.Reminders.Warning.MultiStore"));
    ...
    public virtual async Task<IActionResult> ProcessPendingOrdersTaskWarning(bool enabled)
    {
        ...
        var task = await _scheduleTaskService.GetTaskByTypeAsync(typeof(ProcessPendingOrdersTask).FullName);
